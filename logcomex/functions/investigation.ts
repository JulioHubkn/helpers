...
const investigateMonthlyPerformance = async (
  deals: ICommonDeal[],
  ...
  let endTimestamp = moment(startTimestamp)
    ...
    .valueOf();
  //  ---------------------------------------------------------------------------------------------
  let dealsInTheMonth = await filterDealsArray(
    deals,
    startTimestamp,
    endTimestamp,
    pipeline,
  ...
  //  ---------------------------------------------------------------------------------------------
  let closedValue = await closedDealsTotalValue(
    dealsInTheMonth,
    startTimestamp,
    endTimestamp
  );
  console.log(
    `Valor fechado em ${moment(startTimestamp).format(
      "MM-YYYY"
    )} > ${closedValue}`
  );
  ...
  let lostDealsCount = await lostDealsFilter(
    deals,
    "6647644",
    "6647614",
    "7417228",
...
const filterDealsArray = async (
  ...
  let dealsInTheMonth = deals.filter((e) => {
    const { hs_closed_amount_in_home_currency, pipeline, dealstage } =
      e.properties;
    ...
    if (pipeline) {
      if (dealstage) {
        for (let item of dealstage.versions) {
          ...
          if (
            inBetween(endTimestamp, startTimestamp, item.timestamp) &&
            +item.value === +prospectStageId &&
            +pipeline.value === +pipe
          ) {
            countProspects++;
  ...
  console.log(
    `Prospects gerados 2 nos meses anteriores > ${countProspectsLast2Months}`
  );
  console.log(`Prospects gerados nos mes > ${countProspects}`);
  return dealsInTheMonth;
};

const closedDealsTotalValue = async (
  deals: ICommonDeal[],
  startTimestamp: number,
  endTimestamp: number
) => {
...
const lostDealsFilter = async (
  ...
  let daysFromProspect: byChannel = {
    general: 0,
    Inbound: 0,
    Outbound: 0,
  };
  let daysFromMeeting: byChannel = {
    general: 0,
    Inbound: 0,
    Outbound: 0,
  };
  let countFromProspect: byChannel = {
    general: 0,
    Inbound: 0,
    Outbound: 0,
  };
  let countFromMeeting: byChannel = {
    general: 0,
    Inbound: 0,
    Outbound: 0,
  };
  let countProspects: byChannel = {
    general: 0,
    Inbound: 0,
    Outbound: 0,
  };
  ...
  lostDeals.map((d) => {
    ...
    if (+prospectDate > 0) {
      daysFromProspect.general
        ? (daysFromProspect.general += moment(lostDate).diff(
            prospectDate,
            "days"
          ))
        : (daysFromProspect.general = moment(lostDate).diff(
      ...
      if (canal_de_aquisicao) {
        if (canal_de_aquisicao.value === "Inbound") {
          daysFromProspect.Inbound
            ? (daysFromProspect.Inbound += moment(lostDate).diff(
                prospectDate,
                "days"
              ))
            : (daysFromProspect.Inbound = moment(lostDate).diff(
        ...
        if (canal_de_aquisicao.value === "Outbound") {
          daysFromProspect.Outbound
            ? (daysFromProspect.Outbound += moment(lostDate).diff(
                prospectDate,
                "days"
              ))
            : (daysFromProspect.Outbound = moment(lostDate).diff(
    ...
    if (+meetingDate > 0) {
      daysFromMeeting.general
        ? (daysFromMeeting.general += moment(lostDate).diff(
            meetingDate,
            "days"
          ))
        : (daysFromMeeting.general = moment(lostDate).diff(
      ...
      if (canal_de_aquisicao) {
        if (canal_de_aquisicao.value === "Inbound") {
          daysFromMeeting.Inbound
            ? (daysFromMeeting.Inbound += moment(lostDate).diff(
                meetingDate,
                "days"
              ))
            : (daysFromMeeting.Inbound = moment(lostDate).diff(
        ...
        if (canal_de_aquisicao.value === "Outbound") {
          daysFromMeeting.Outbound
            ? (daysFromMeeting.Outbound += moment(lostDate).diff(
                meetingDate,
                "days"
              ))
            : (daysFromMeeting.Outbound = moment(lostDate).diff(
